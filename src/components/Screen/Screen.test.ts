...
import type { ScreenKeyboardEvent } from './Screen';
...
describe('Screen unit test', () => {
  ...
  it('Should emit current screenHeight & lastClick values as payload for keyboardBlur event when window is resized', () => {
    ...
    window.dispatchEvent(new Event('resize'));
    const [keyboardBlurEvent] = wrapper.emitted('keyboardBlur')?.[0] as ScreenKeyboardEvent[];
  ...
  it('Should record last click when its clicked', async () => {
    const wrapper = mount(Screen, { slots: { default: '<h1 data-testid="screen-content">Content</h1>' } });
    const section = wrapper.find('section');
    await section.trigger('click', { clientX: 200, clientY: 200 });
    const dataFromLastClick = wrapper.vm.lastClick;
  ...
  it('Should send recorded last clicks as payload for emitted event when window is resized', async () => {
    const wrapper = mount(Screen, { slots: { default: '<h1 data-testid="screen-content">Content</h1>' } });
    const section = wrapper.find('section');
    await section.trigger('click', { clientX: 200, clientY: 200 });
    window.dispatchEvent(new Event('resize'));
    const [keyboardBlurEvent] = wrapper.emitted('keyboardBlur')?.[0] as ScreenKeyboardEvent[];
    const emittedLastClickX = keyboardBlurEvent?.lastClick.x;
    const emittedLastClickY = keyboardBlurEvent?.lastClick.y;
