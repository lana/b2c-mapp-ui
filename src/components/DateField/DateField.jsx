...
import { getDateFromDateString, getFormattedStringFromDate, autoformatDate } from '../../utils/utils';
...
const DateField = ({ autoformat, className, dataTestId, datepicker, errorLabel, id, label, maxLength, name, readOnly, value, onChange, onBlur, onFocus, onValidation }) => {
  ...
  const handleOnChange = (eventDate) => {
    const eventValue = (autoformat) ? autoformatDate(eventDate) : eventDate;
    const newValidation = (dateRegex.test(eventValue)) ? 'success' : 'error';
    const calValue = (newValidation === 'success') ? getDateFromDateString(eventValue) : eventValue;
  ...
  const handleCalendarChange = (event) => {
    ...
    const formattedDate = getFormattedStringFromDate(new Date(eventValue));
