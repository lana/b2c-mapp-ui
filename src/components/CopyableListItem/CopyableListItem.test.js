...
describe('CopyableListItem unit test', () => {
  ...
  it('Should show given title', () => {
    const { getByTestId } = render(CopyableListItem, { stubs: { default: defaultSlot }, props: { ...defaultProps, hideButton: false } });
  ...
  it('Should display copy to clipboard button if hide property is false for given option', () => {
    const { queryAllByTestId } = render(CopyableListItem, { stubs: { default: defaultSlot }, props: { ...defaultProps, hideButton: false } });
  ...
  it('Should NOT display copy to clipboard button if hide property is true for given option', () => {
    const { queryAllByTestId } = render(CopyableListItem, { stubs: { default: defaultSlot }, props: { ...defaultProps, hideButton: true } });
  ...
  it('Should display copy to clipboard button if hide property is NOT given in option', () => {
    const { queryAllByTestId } = render(CopyableListItem, { stubs: { default: defaultSlot }, props: { ...defaultProps } });
    const buttonExists = queryAllByTestId('copyable-list-item-copy-to-clipboard-button-button').length;
    expect(buttonExists).toBeTruthy();
  });

  it('should emit an event when is clicked', () => {
    const { getByTestId, emitted } = render(CopyableListItem, { stubs: { default: defaultSlot }, props: { ...defaultProps } });
