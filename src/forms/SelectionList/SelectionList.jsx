...
import CSS from './SelectionList.css'
...
export default class SelectionList extends Component {
	...
	componentWillReceiveProps(props) {
		let initialValue = props.options.reduce((buffer, option) => (option.selected) ? buffer + option.value : buffer, '')
	...
	render(props) {
		return (
			<section className={`${CSS.wrapper} ${props.className || ''}`}>
				{props.title && <Heading className={CSS.title} type='callout'>{props.title}</Heading> }
				<ul>
					{props.options.map((option, index) => {
						...
						return (
							<li key={index} className={`${CSS.item} ${(checked) ? CSS.checked : '' }`} onClick={e => this.onClick(option, index)}>
								<input type='radio' className={CSS.radio} id={id} checked={checked} name={props.id} value={option.value}/>
								<div className={CSS.content}>
									{ (option.label) ? <label className={CSS.label} htmlFor={id}>{option.label}</label> : option.children }
								</div>
							</li>
						)
					})}
